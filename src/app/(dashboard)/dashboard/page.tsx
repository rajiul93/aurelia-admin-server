...
import { STAT_CARD_SHELL, SectionHeading } from './dashboard-ui';
import { DeviceAccessAnalyticsSection } from './device-access-analytics';
...
function QuickActionCard({ item, index }: { item: NavItem; index: number }) {
  if (!item.href) {
    return null;
  }
  ...
  return (
    <Link href={item.href}>
      <Card className={cn(STAT_CARD_SHELL, 'h-full hover:ring-brand-tan/60')}>
        <div className={cn('h-0.5 bg-linear-to-r', accent.bar)} aria-hidden />
...
function HandbookSectionCard({ section }: { section: HandbookSection }) {
  ...
  return (
    <AccordionItem value={section.id} className="border-border/50 px-4 md:px-6">
...
function RoleBadge({ role }: { role: StaffRole }) {
  const styles: Record<StaffRole, { className: string; dot: string }> = {
    SUPERADMIN: {
